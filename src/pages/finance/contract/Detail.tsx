...
import { getReceiptDetail, type ReceiptDetail } from '@/api/payment';
import { getAccountAdjustRecords } from '@/api/classHour';
import type { ClassHourAdjustRecord } from '@/types/classHour';
import type {
  Contract,
  ContractItem,
  PaymentRecord,
...
const ContractDetail: React.FC = () => {
  ...
  const [receiptPrintPaymentId, setReceiptPrintPaymentId] = useState<number | null>(null);

  // 消课记录相关状态
  const [consumptionModalVisible, setConsumptionModalVisible] = useState(false);
  const [consumptionLoading, setConsumptionLoading] = useState(false);
  const [consumptionRecords, setConsumptionRecords] = useState<ClassHourAdjustRecord[]>([]);
  const [consumptionCourseName, setConsumptionCourseName] = useState('');
  ...
  const handlePrintReceipt = (paymentId: number) => {
    setReceiptPrintPaymentId(paymentId);
    setReceiptPrintVisible(true);
  };

  // 查看消课记录
  const handleViewConsumption = async (accountId: number) => {
    const account = hourAccounts.find((a) => a.id === accountId);
    setConsumptionCourseName(account?.courseName || '');
  ...
  return (
    <div className="contract-detail">
          ...
          {receiptData ? (
            ...
            >
              ...
              <Descriptions.Item label="收款方式">
                <Tag color="blue">{receiptData.paymentMethod}</Tag>
              </Descriptions.Item>
              <Descriptions.Item label="学员姓名">
                {receiptData.studentInfo.name}
              </Descriptions.Item>
              <Descriptions.Item label="联系电话">
                {receiptData.studentInfo.phone}
              </Descriptions.Item>
              <Descriptions.Item label="学员编号" span={2}>
                {receiptData.studentInfo.studentNo}
              </Descriptions.Item>
              <Descriptions.Item label="收款项目">
                {receiptData.paymentItem.name}
              </Descriptions.Item>
              <Descriptions.Item label="合同编号">
                {receiptData.paymentItem.contractNo}
              </Descriptions.Item>
              <Descriptions.Item label="数量">
                {receiptData.paymentItem.quantity}
              </Descriptions.Item>
              <Descriptions.Item label="单价">
                ¥{receiptData.paymentItem.unitPrice.toFixed(2)}
              </Descriptions.Item>
              <Descriptions.Item label="收款金额" span={2}>
                <span style={{ color: '#00ff88', fontWeight: 'bold', fontSize: 18 }}>
      ...
      {/* 打印收据弹窗 */}
      <ReceiptPrint
        visible={receiptPrintVisible}
        paymentId={receiptPrintPaymentId}
        onClose={() => {
          setReceiptPrintVisible(false);
          setReceiptPrintPaymentId(null);
        }}
      />

      {/* 消课记录弹窗 */}
      <Modal
        title={`消课记录${consumptionCourseName ? ` - ${consumptionCourseName}` : ''}`}
          ...
          <Table<ClassHourAdjustRecord>
            columns={[
              ...
              },
              {
                ...
                render: (type: string) => {
                  const typeMap: Record<string, { color: string; text: string }> = {
                    gift: { color: '#00d4ff', text: '赠送' },
                    deduct: { color: '#ffaa00', text: '消课' },
                    revoke: { color: '#ff4d6a', text: '撤销' },
                  };
                  const config = typeMap[type] || { color: '#999', text: type };
                  return (
                    <Tag
                      style={{
                        background: `${config.color}20`,
              ...
              },
              {
                ...
                align: 'right',
                render: (value: number) => (
                  <span style={{ color: value > 0 ? '#00ff88' : '#ff4d6a', fontWeight: 'bold' }}>
                    {value > 0 ? `+${value}` : value}
                  </span>
                ),
              },
            ...
            rowKey="id"
            pagination={consumptionRecords.length > 10 ? { pageSize: 10 } : false}
            scroll={{ x: 800 }}
            locale={{ emptyText: '暂无消课记录' }}
          />
        </Spin>
      </Modal>
    </div>
