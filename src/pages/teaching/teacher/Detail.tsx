...
import type { Teacher, Certificate, Schedule, CertificateFormData, ScheduleFormData, SalaryConfig, SalaryConfigFormData, ScheduleEvent } from '@/types/teacher';
import {
  getTeacherDetail,
...
export function Component() {
  ...
  const handleSalarySubmit = async () => {
    try {
      ...
      const formData: SalaryConfigFormData = {
        ...values,
        teacherId,
      ...
      if (editingSalary) {
        await updateSalaryConfig(editingSalary.id, formData);
        message.success('更新成功');
      } else {
        await createSalaryConfig(formData);
        message.success('创建成功');
      }
  ...
  const handleAvailableTimeSubmit = async () => {
    try {
      ...
      const scheduleDataList: ScheduleFormData[] = [];
      ...
      Object.keys(weekDayMap).forEach((key) => {
        ...
        if (dayData?.startTime && dayData?.endTime) {
          scheduleDataList.push({
            ...
            startTime: dayData.startTime.format('HH:mm'),
            endTime: dayData.endTime.format('HH:mm'),
            campusId: teacher?.campusId || 0,
          });
        }
      });

      await batchConfigAvailableTime(teacherId, scheduleDataList);
