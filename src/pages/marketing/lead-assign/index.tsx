...
export function Component() {
  ...
  const handleQuickAssign = async () => {
    ...
    // 取选中线索中第一条的校区ID，若筛选了校区则用筛选值
    const campusId = queryParams.campusId
      || dataSource.find((d) => selectedRowKeys.includes(d.id))?.campusId;

    if (!campusId) {
      message.warning('无法确定校区，请先筛选校区或选择含校区信息的线索');
      return;
    }

    setAssigning(true);
    try {
      await autoAssignLeads(selectedRowKeys, campusId);
