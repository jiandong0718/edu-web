...
import { downloadStudentTemplate, importStudents } from '@/api/student';
import type { StudentImportResult } from '@/types/student';

export interface StudentBatchImportProps {
  open: boolean;
  onCancel: () => void;
  onSuccess?: () => void;
}
...
export function StudentBatchImport({ open, onCancel, onSuccess }: StudentBatchImportProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [fileList, setFileList] = useState<UploadFile[]>([]);
  const [uploading, setUploading] = useState(false);
  const [importResult, setImportResult] = useState<StudentImportResult>();
    ...
    >
        ...
        description={
          <div>
            <p style={{ marginBottom: 8 }}>
              1. 请先下载导入模板，按照模板格式填写学员信息
            </p>
            <p style={{ marginBottom: 8 }}>
              2. 必填字段：姓名、性别、手机号、家长姓名、家长电话、出生日期、地址、学校、年级、所属校区、入学日期
            </p>
            <p style={{ marginBottom: 8 }}>3. 性别填写：男 或 女</p>
            <p style={{ marginBottom: 8 }}>4. 日期格式：YYYY-MM-DD（例如：2010-01-01）</p>
            <p style={{ marginBottom: 8 }}>5. 所属校区填写校区ID（可在校区管理中查看）</p>
            <p style={{ marginBottom: 0 }}>6. 单次最多导入 1000 条数据</p>
          </div>
        }
        type="info"
        showIcon
      ...
      {currentStep === 1 && (
        <div style={styles.confirmContainer}>
          <CheckCircleOutlined style={styles.confirmIcon} />
          <div style={styles.confirmTitle}>文件上传成功，准备导入</div>
          <div style={styles.confirmSubtitle}>文件名：{fileList[0]?.name}</div>
          <Space size="large">
      ...
      {currentStep === 2 && importResult && (
        <>
          <Result
            ...
            title={
              importResult.failed === 0 ? '导入成功！' : '导入完成，部分数据导入失败'
            }
            subTitle={
              <div>
            ...
            extra={[
              ...
              <Button key="close" type="primary" onClick={handleClose} style={styles.actionButton}>
                完成
              </Button>,
            ]}
          />
          ...
          {importResult.errors && importResult.errors.length > 0 && (
            <div style={{ marginTop: 24 }}>
              ...
              >
                <CloseCircleOutlined style={{ color: '#ff4d6a' }} />
                错误详情
              </div>
              <Table
...
export default StudentBatchImport;
